...
function cloudPing (urlServer, urlFeed) {
	...
	request.post (rq, function (err, res, body) {
		console.log ("cloudPing: urlServer == " + urlServer + ", urlFeed == " + urlFeed + ", res.statusCode == " + res.statusCode);
		});
	}
function chatLogToRss (headElements, chatlog) {
	...
	for (var i = chatlog.length - 1; i >= 0; i--) {
		...
		if ((chatitem.payload !== undefined) && (chatitem.payload.urlRendering !== undefined)) { //it has been published
