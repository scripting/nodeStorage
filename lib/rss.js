...
function buildRssFeed (headElements, historyArray) {
	...
	var username = headElements.twitterScreenName, maxitems = headElements.maxFeedItems, contentNamespaceDecl = "";
	function add (s) {
	...
	add ("<rss version=\"2.0\" xmlns:source=\"http://source.smallpict.com/2014/07/12/theSourceNamespace.html\"" + contentNamespaceDecl + ">"); indentlevel++
		...
		function getOutline (chatitem) {
			...
			if (chatitem.subs !== undefined) {
				...
				for (var i = 0; i < chatitem.subs.length; i++) {
					theOutline.subs [i] = getOutline (chatitem.subs [i]);
					}
				}
			return (theOutline);
			}
