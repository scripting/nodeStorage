...
function getFolderInfo (folderpath, folderInfoCallback) { //3/2/20 by DW
...
var filesCurrentlyBeingWritten = { //9/27/16 by DW -- see this post -- http://scripting.com/2016/09/27/whatILearnedAboutFswritefileToday.html
	};

function myNewObject (path, data, type, acl, callback, metadata) {
	if (filesCurrentlyBeingWritten [path] !== undefined) { //9/27/16 by DW
		if (callback !== undefined) {
			var err = {
				message: "Can't write the file \"" + path + "\" because the file is being written by another thread at the same time."
				};
			console.log ("myNewObject: err.message == " + err.message);
			var dataAboutWrite = {};
			callback (err, dataAboutWrite);
			}
		return;
	...
	fsSureFilePath (path, function () {
		filesCurrentlyBeingWritten [path] = true;
		fs.writeFile (path, data, function (err) {
			delete filesCurrentlyBeingWritten [path];
			var dataAboutWrite = {
				};
			if (err) {
				console.log ("fsNewObject: error == " + JSON.stringify (err, undefined, 4));
